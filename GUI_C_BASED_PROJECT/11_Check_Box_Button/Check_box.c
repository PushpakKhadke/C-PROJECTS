...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        // Create checkbox
        hwndCheckbox = CreateWindow(TEXT("BUTTON"), TEXT("Check Box"),
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == 1 && HIWORD(wParam) == BN_CLICKED) {
            ...
            BOOL isChecked = SendMessage(hwndCheckbox, BM_GETCHECK, 0, 0);
            if (isChecked == BST_CHECKED) {
                MessageBox(hwnd, TEXT("Checkbox is checked!"), TEXT("Checkbox Checked"), MB_OK | MB_ICONINFORMATION);
            } else {
                MessageBox(hwnd, TEXT("Checkbox is not checked!"), TEXT("Checkbox Not Checked"), MB_OK | MB_ICONINFORMATION);
            }
        }
        return 0;
